...
class BTree {
	...
	private int numLeavesH(Node current) {
		if (current == null){
			return 0;
		}
		else {
			if (current.left == null && current.right == null){
				...
				return numLeavesH(current.right) + numLeavesH(current.left);
			}
		}
	}
	public int level(Comparable key){
		return levelh(root , key);
	}

	private int levelh(Node current, Comparable key) {
		if (current == null){
			return -1;
		}
		else {
			if (key.equals(current.data)){
				...
				return 1 + levelh(current.right, key) + levelh(current.left, key);
			}
		}
	}
	public boolean equal(BTree t2){
		return equalsh(this.root, t2.root);
	}

	private boolean equalsh(Node node1, Node node2) {
		if (node1 == null && node2 == null){
			return true;
		}
		else {
			if (node1.data.compareTo(node2.data)>0 || node1.data.compareTo(node2.data)<0) {
				return false;
			}
			else {
				return equalsh(node1.right, node2.right) && equalsh(node1.left, node2.left);
			}
		}
	}
}